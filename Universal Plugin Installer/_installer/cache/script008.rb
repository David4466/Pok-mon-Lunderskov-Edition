...
def pbTransferWithTransition(map_id,x,y,transition=nil,dir=$game_player.direction)
  ...
  if $game_temp.player_transferring or $game_temp.message_window_showing or $game_temp.transition_processing
    return false
  end
  ...
  if transition != :ZOOMED
    screen = Sprite.new(viewport)
    screen.bitmap = bmp
  end
  # 4 frame wait
  4.times do
    next if transition == :ZOOMED
    Graphics.update
  end
  ...
  screen.visible = false if transition != :ZOOMED
  ...
  screen.dispose if transition != :ZOOMED
  viewport.dispose
end
#===============================================================================  
...
module Transitions
  ...
  def self.directed(reverse = false)
    ...
    for i in 0...16
      fp["trans"].x -= (Graphics.width/8)*(flip ? -1 : 1)*(horizontal ? 1 : 0)*(reverse ? -1 : 1)
      fp["trans"].y -= (Graphics.height/8)*(flip ? -1 : 1)*(horizontal ? 0 : 1)*(reverse ? -1 : 1)
      fp["screen"].opacity += 16*(reverse ? -1 : 1)
      Graphics.update
    end
    bmp = Graphics.snap_to_bitmap
  ...
  def self.circular(reverse = false)
    ...
    for i in 0...4
      bmp.drawCircle(Color.new(0,0,0,120 - 40*i),bmp.height/2 - 12*i)
    end
    ...
    for i in 0..20
      ...
      sprite.opacity += 48
      zoom -= 2.0/20*(reverse ? -1 : 1)
      bg.opacity += 16*(reverse ? -1 : 1)
      Graphics.update
    end
    bmp = Graphics.snap_to_bitmap
    bg.dispose
  ...
  def self.fade(reverse=false)
    ...
    8.times do
      viewport.color.alpha += 32*(reverse ? -1 : 1)
      Graphics.update
    end
    bmp = Graphics.snap_to_bitmap
    viewport.dispose
