...
class ::String
  ...
  def ends_with?(char)
    proc = self.scan(/./)
    return (proc[proc.length-1] == char) ? true : false
  end
...
class Viewport
  ...
  def flatten
    s = self.getAllSprites
    for key in s.keys
      sprite = s[key]
    end
    bitmap = Bitmap.new(self.rect.width,self.rect.height)
    for key in sprites.keys
      sprite = sprites[key]
      next if !sprite.respond_to?(:x) || !sprite.respond_to?(:y) || !sprite.respond_to?(:bitmap)
